...
;(function () {
  ...
  function format(str, args) {
    const keys = Object.keys(args); let i
  ...
  function createNamedToNumberedLookup(items, radix) {
    let i; let entity; const lookup = {}; let base10; let base16
    items = items.split(',')
    radix = radix || 10
  ...
  CanvasGradient.prototype.addColorStop = function (offset, color) {
    const stop = this.__ctx.__createElement('stop'); let regex; let matches
   ...
   */
  ctx = function (o) {
    const defaultOptions = { width: 500, height: 500, enableMirroring: false }; let options
  ...
  ctx.prototype.__createElement = function (elementName, properties, resetFill) {
    ...
    const keys = Object.keys(properties); let i; let key
    if (resetFill) {
  ...
  ctx.prototype.__setDefaultStyles = function () {
    ...
    const keys = Object.keys(STYLES); let i; let key
    for (i = 0; i < keys.length; i++) {
      key = keys[i]
  ...
  ctx.prototype.__applyStyleState = function (styleState) {
    const keys = Object.keys(styleState); let i; let key
    for (i = 0; i < keys.length; i++) {
      key = keys[i]
  ...
  ctx.prototype.__getStyleState = function () {
    let i; const styleState = {}; const keys = Object.keys(STYLES); let key
    for (i = 0; i < keys.length; i++) {
      key = keys[i]
  ...
  ctx.prototype.__applyStyleToCurrentElement = function (type) {
    ...
    const keys = Object.keys(STYLES); let i; let style; let value; let id; let regex; let matches
    for (i = 0; i < keys.length; i++) {
      style = STYLES[keys[i]]
      value = this[keys[i]]
      if (style.apply) {
  ...
  ctx.prototype.getSerializedSvg = function (fixNamedEntities) {
    ...
    let keys; let i; let key; let value; let regexp; let xmlns
  ...
  ctx.prototype.clearRect = function (x, y, width, height) {
    ...
    let rect; const parent = this.__closestGroupOrSvg()
    rect = this.__createElement('rect', {
      x,
      y,
      width,
  ...
  ctx.prototype.arc = function (x, y, radius, startAngle, endAngle, counterClockwise) {
    ...
    this.__addPathCommand(format('A {rx} {ry} {xAxisRotation} {largeArcFlag} {sweepFlag} {endX} {endY}', { rx: radius, ry: radius, xAxisRotation: 0, largeArcFlag, sweepFlag, endX, endY }))
  ...
  ctx.prototype.drawImage = function () {
    ...
    let image = args[0]
    let dx; let dy; let dw; let dh; let sx = 0; let sy = 0; let sw; let sh; let parent; let svg; let defs; let group
    let currentElement; let svgImage; let canvas; let context; let id
  ...
  ctx.prototype.createPattern = function (image, repetition) {
    const pattern = this.__document.createElementNS('http://www.w3.org/2000/svg', 'pattern'); const id = randomString(this.__ids)
    let img
